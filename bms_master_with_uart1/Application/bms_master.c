...
static void BmsLoop(uintptr_t a0, uintptr_t a1){
    uint8_t i;
    /*
    uint8_t data[20] = "Eximius Design\r\n";

    while(1){
    UART_write(uart, &data, 20);
    Task_sleep(CLK_SEC *  3);
    }
     */
...
static void BmsMaster_processAppMsg(bmsEvt_t *pMsg)
{
    ...
    case BMS_EVT_SENT_UART:
    {
        ...
        //if(numConn){
        uint8_t i,j;
        uint8_t t_data[256] = {0};
        uint8_t t_len = 0;
        for (i = 0; i < MAX_NUM_BLE_CONNS; i++){
            bms_uart_data[i].flag = 1;
            memcpy(bms_uart_data[i].data, "This is 20 byte data", 20);
        }
